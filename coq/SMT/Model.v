...
Fixpoint normalize (e : smt_expr) : smt_expr :=
match e with
  | SMT_BinOp op e1 e2 => SMT_BinOp op (normalize e1) (normalize e2)
  | SMT_CmpOp op e1 e2 =>
    match op with
    | SMT_Sge => SMT_CmpOp SMT_Sle (normalize e2) (normalize e1)
    | SMT_Sgt => SMT_CmpOp SMT_Slt (normalize e2) (normalize e1)
    | SMT_Ugt => SMT_CmpOp SMT_Ult (normalize e2) (normalize e1)
    | SMT_Uge => SMT_CmpOp SMT_Ule (normalize e2) (normalize e1)
    | op => SMT_CmpOp op (normalize e1) (normalize e2)
    end
  | SMT_Not e => SMT_CmpOp SMT_Eq SMT_False (normalize e)
  | _ => e
  end
.

Lemma equiv_smt_expr_normalize : forall e,
  equiv_smt_expr e (normalize e).
Proof.
Admitted.
...
Fixpoint simplify (e : smt_expr) : smt_expr :=
  ...
  | SMT_CmpOp op e1 e2 =>
    ...
    | SMT_Eq =>
      match (simplify e1), (simplify e2) with
      | SMT_Const_I1 n1, SMT_Const_I1 n2 => make_smt_bool (eq n1 n2)
      | SMT_Const_I8 n1, SMT_Const_I8 n2 => make_smt_bool (eq n1 n2)
      | SMT_Const_I16 n1, SMT_Const_I16 n2 => make_smt_bool (eq n1 n2)
      | SMT_Const_I32 n1, SMT_Const_I32 n2 => make_smt_bool (eq n1 n2)
      | SMT_Const_I64 n1, SMT_Const_I64 n2 => make_smt_bool (eq n1 n2)
      | e1, e2 => SMT_CmpOp SMT_Eq e1 e2
      end
    | SMT_Ne (* TODO: is it generated by the engine? *)
    | SMT_Slt
    | SMT_Sle =>
      match (simplify e1), (simplify e2) with
